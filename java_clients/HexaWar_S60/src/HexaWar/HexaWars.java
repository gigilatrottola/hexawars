...
public class HexaWars extends MIDlet {
    ...
    class GameCanvas extends FullCanvas implements Runnable {
        ...
        final static byte TILE_VISIBLE_Y = 6;
        ...
        final static byte INFO_LINES=12;
        ...
        final static byte DISPLAY_HELP_LINES = 12;
...
//#endif
        ...
        public void paint_footer(String s1, String s2, int color) {
            ...
            paint_shaded(s1, color, 2, SH-15, false);
            paint_shaded(s2, color, SW-2-g2.getFont().stringWidth(s2), SH-15, false);
        }
        ...
        public void paint_arrow(int posx, int posy, int direction, int color) {
            ...
            g2.drawImage(img[15], xp-startOfs[direction], yp-color*(img[15].getHeight()>>3), Graphics.TOP|Graphics.LEFT);
        }
        ...
        public void paint_game_scene(boolean paintSelection, boolean paintFooter) {
            g2.setClip(0,0,SW,SH);
            int SQUAD_ANIM_Y = img[19].getHeight()>>3;
            int SQUAD_ANIM_X = img[19].getWidth()>>2;
            ...
            if (scene==RUNNING_GAME_SCENE && keypressed[MY_KEY_POUND]) {
                ...
                for (ofs=0; ofs<squads.length; ofs+=SQUAD_H) {
                    ...
                    g2.drawImage(img[17], xp-players[squads[ofs+JATEKOS]]*6, yp, Graphics.TOP|Graphics.LEFT);
                }
                paint_gui(true);
                return;
            }
            ...
            for (i=0, ofs=0; ofs<squads.length; ofs+=SQUAD_H, ++i) {
                ...
                g2.drawImage(img[19], xp, yp-SQUAD_ANIM_Y*players[squads[ofs+JATEKOS]], Graphics.TOP|Graphics.LEFT);
            }
...
//          initCsataAnim();
        ...
        public void background(boolean logo) {
            ...
            g2.drawImage(img[0], img[0].getWidth(), 0, Graphics.TOP|Graphics.LEFT);
            g2.drawImage(img[0], 0, img[0].getHeight(), Graphics.TOP|Graphics.LEFT);
            g2.drawImage(img[0], img[0].getWidth(), img[0].getHeight(), Graphics.TOP|Graphics.LEFT);
            if (logo) {
        ...
        public void paint_menu_txt(String txt[], int x, int y, int width,
                int step, boolean center, int start) {
            int nx;
            g2.setClip(0,0,SW,SH);
            for (int i=start; i<txt.length; ++i, y+=step) {
        ...
        public void paint_connection_panel(String s, int size) {
            fill_shaded_rect(0x355046, (SW-size)>>1, SH-80, size, 25);
            int pos=SW_p2-(g2.getFont().stringWidth(s)>>1);
            for (int i=0; i<(frame/5)%4; ++i) {
                s+=".";
            }
            paint_shaded(s, 0xFFFFFF, pos, SH-80+5, false);
        }
        ...
        public void paint_divide_panel() {
            g2.setClip(0,0,SW,SH);
            fill_shaded_rect(0x355046, SW_p2-70, SH_p2-60, 140, 120);
            
            for (int i=0; i<3; ++i) {
                g2.setClip(SW_p2-40, SH_p2-8+i*17, 6, 8);
                g2.drawImage(img[18], SW_p2-40-6*i, SH_p2-8+i*17, Graphics.TOP|Graphics.LEFT);
                g2.setClip(0,0,SW,SH);
                g2.setColor(0);
                ...
                paint_shaded(""+darabolEgyseg[i], 0xFFFFFF, SW_p2-25, SH_p2-10+i*17, false);
                paint_shaded(divide[1], 0xFFFFFF, SW_p2+5, SH_p2-10+i*17, false);
                paint_shaded(""+squads[canSelectUnit*SQUAD_H+i], 0xFFFFFF, SW_p2+25, SH_p2-10+i*17, false);
            }
            paint_shaded(divide[0], 0xFFFFFF, 0, SH_p2-55, true);
        }
        ...
        public void paint_table(int startx, int starty, int accountTxt, int newLoginOfs) {
            ...
            g2.setColor(0xFF0000);
            g2.drawRect(startx+(cursorPos%CHAR_PER_LINE)*betukoz-5, starty+(cursorPos/CHAR_PER_LINE)*betukoz-1, betukoz, betukoz);
            paint_shaded(account[accountTxt], 0xFFFFFF, 55-g2.getFont().stringWidth(account[accountTxt]), SH-30, false);
            g2.setClip(55, SH-30, SW-60, 15);
            g2.setColor(0xFFFFFF);
            g2.drawString(newLogin[newLoginOfs].toString(), SW-5, SH-30, Graphics.TOP|Graphics.RIGHT);
        }
        ...
        public void paint_name(int x, int y, int i, int SQUAD_ANIM_X, int SQUAD_ANIM_Y) {
            g2.setClip(x, y, SQUAD_ANIM_X, SQUAD_ANIM_Y);
            g2.drawImage(img[19], x-anim_phase_had[frame%4], y-SQUAD_ANIM_Y*players[i], Graphics.TOP|Graphics.LEFT);
            g2.setClip(0,0,SW,SH);
            paint_shaded(server_jatekosnev[i], 0xFFFFFF, x+33+10, y+7, false);            
        }
        
        public void paint_numbers(int x, int y, int i, int SQUAD_ANIM_X, int SQUAD_ANIM_Y) {
            g2.setClip(x, y, SQUAD_ANIM_X, SQUAD_ANIM_Y);
            g2.drawImage(img[19], x-anim_phase_had[frame%4], y-SQUAD_ANIM_Y*players[i], Graphics.TOP|Graphics.LEFT);
            g2.setClip(0,0,SW,SH);
            x+=40;
            paint_shaded(""+server_sereg_vegeredmeny[i], 0xFFFFFF, x, y+8, false);
            x+=30;
            g2.setClip(x, y, castleSizeX[0], castleSizeY[0]);
            g2.drawImage(img[6], x-players[i]*castleSizeX[0], y, Graphics.TOP|Graphics.LEFT);
            x+=35;
            g2.setClip(0,0,SW,SH);
            paint_shaded(""+server_varak_vegeredmeny[i], 0xFFFFFF, x, y+8, false);            
        }
        
        public void paint(Graphics g) {
            int x=0, y=0, i, SQUAD_ANIM_Y=img[19].getHeight()>>3;
            int SQUAD_ANIM_X = img[19].getWidth()>>2;
            if (g2!=g) {
                g2=g;
            }
            ...
            switch (scene) {
                case RULES_SCENE:
                    ...
                    for (y=0, i=menu_selected; y<DISPLAY_HELP_LINES; ++i, ++y) {
                        paint_shaded(rulesArray[i], 0xFFFFFF, 0, y*15+5, true);
                    }
                    if (menu_selected+DISPLAY_HELP_LINES<rulesArray.length) {
                ...
                case HOWTOPLAY_SCENE:
                    background(false);
                    for (y=0, i=menu_selected; y<DISPLAY_HELP_LINES; ++i, ++y) {
                        paint_shaded(h2pArray[i], 0xFFFFFF, 0, y*15+5, true);
                    }
                    if (menu_selected+DISPLAY_HELP_LINES<h2pArray.length) {
                        paint_down_arrow(SW_p2-10, SH-10);
                    }
                ...
                case WAIT_FOR_GAME_SCENE:
                    background(true);
                    paint_connection_panel(other_txt[3], SW-30);
                    break;
                    
                case SHOW_TYPE_SCENE:
                    background(true);
                    if (!typeReaded) {
                        paint_connection_panel(other_txt[1], SW-30);
                    } else {
                        String[] tmp=new String[server_gtypenum];
                        for (i=0; i<tmp.length; ++i) {
                            tmp[i]=server_gtpname[i];
                        }
                        paint_menu_txt(tmp, SW-125, SH-server_gtypenum*17, 123, 17, true, 0);
                        paint_footer(other_txt[2], "", 0xFFFFFF);
                    }
                    break;
                ...
                case WAIT_AND_TRY_AGAIN:
                    background(true);
                    fill_shaded_rect(0x355046, 15, SH-80, SW-30, 45);
                    y=SH-80+5;
                ...
                case GET_REG_INFO_SCENE:
                    background(false);
                    paint_connection_panel(other_txt[11], SW-30);
                    break;
                case REG_ERROR_SCENE:
                    background(false);
                ...
                case MAIN_MENU_SCENE:
                    background(true);
                    paint_menu_txt(menu_txt_1, SW-125, SH-5*17, 123, 17, true, 0 );
                    break;
                ...
                case ACCOUNT_SUBMENU_SCENE:
                    ...
                    g.drawImage(img[11], 2+(img[11].getWidth()<<1), 2, Graphics.TOP|Graphics.LEFT);
                    g.setClip(0,0,SW,SH);
                    g.setColor(0);
                    ...
                    paint_menu_txt(accountMod, SW-125, SH-4*17, 123, 17, true, 0);
                    paint_footer(other_txt[2], "", 0xFFFFFF);
                    if (areyousure) {
                        g.setColor(0x355046);
                        g.fillRect(0, SH-70, SW, 70);
                        paint_shaded(other_txt[8], 0xFFFFFF, 0, SH-65, true);
                        paint_shaded(other_txt[9], 0xFFFFFF, 0, SH-50, true);
                        paint_shaded(other_txt[10], 0xFFFFFF, 0, SH-35, true);
                        paint_footer(yesno[1], yesno[0], 0xFFFFFF);
                    }
                    break;
                ...
                case ACCOUNT_CHANGED:
                    background(false);
                    fill_shaded_rect(0x355046, 15, 30, SW-30, accountChanged.length*15+20);
                ...
                case INPUT_ACCOUNT_MAIL:
                    background(false);
                    paint_table(35, 25, scene-INPUT_ACCOUNT_NAME+1, scene-INPUT_ACCOUNT_NAME);
                ...
                case ACCOUNT_SCENE:
                    ...
                    paint_menu_txt(mntmp, SW-125, SH-3*17, 123, 17, true, 0);
                    ...
                    g.drawImage(img[11], 5+(img[11].getWidth()<<1), 5, Graphics.TOP|Graphics.LEFT);
                    g.setClip(0,0,SW,SH);
                    g.setColor(0);
                    g.drawRect(5, 5, SW-10, 40);
                    if (defaultAccount!=-1 && accountNum!=0) {
                        paint_shaded(account[4], 0xFFFFFF, 0, 10, true);
                        paint_shaded(loginName[defaultAccount], 0xFFFFFF, 0, 25, true);
                    }
                ...
                case LOGIN_SCENE:
                    background(true);
                    paint_connection_panel(other_txt[0], SW-30);
                    break;
                ...
                case SEND_ACTION_SCENE:
                    ...
                    paint_gui(false);
                    paint_connection_panel(other_txt[5], SW-30);
                    move_kamera(true);
                    break;
                case END_OF_GAME_SCENE_2:
                    background(false);
                ...
                case END_OF_GAME_SCENE_1:
                    ...
                    for (i=0; i<Math.min(server_sereg_vegeredmeny.length, server_varak_vegeredmeny.length); ++i) {
                        x=10;
                        g.setClip(x, y, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                        g.drawImage(img[19], x-anim_phase_had[frame%4], y-SQUAD_ANIM_Y*players[server_id_vegeredmeny[i]], Graphics.TOP|Graphics.LEFT);
                        g.setClip(0,0,SW,SH);
                        x+=40;
                ...
                case DISPLAY_END_OF_TURN_INFO_3:
                    ...
                    if (server_hg_vedo_jatekos[eofTurnBattle]==server_hg_sajatsorszam) {
                        ...
                        paint_shaded(attack[2], 0xFFFFFF, 0, SH_p2-58, true);
                        paint_shaded(attack[3], 0xFFFFFF, 0, SH_p2-47, true);
                    } else { // a jatekos tamadott
                        paint_shaded(attack[0], 0xFFFFFF, 0, SH_p2-58, true);
                        paint_shaded(attack[1], 0xFFFFFF, 0, SH_p2-47, true);
                    }
                    ...
                    y=SH_p2-25;
                    g.setClip(x, y, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                    g.drawImage(img[19], x-anim_phase_had[frame%4], y-SQUAD_ANIM_Y*players[server_hg_vedo_jatekos[eofTurnBattle]], Graphics.TOP|Graphics.LEFT);
                    g.setClip(0,0,SW,SH);
                    paint_shaded(a1, 0xFFFFFF, x+33+10, y+4, false); // vedo
                    
                    x=SW_p2-50;
                    y=SH_p2-5;
                    g.setClip(x, y, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                    g.drawImage(img[19], x-anim_phase_had[frame%4], y-SQUAD_ANIM_Y*players[server_hg_tamado_jatekos[eofTurnBattle]], Graphics.TOP|Graphics.LEFT);
                    g.setClip(0,0,SW,SH);
                    paint_shaded(a2, 0xFFFFFF, x+33+10, y+4, false); // tamado
                    ...
                    paint_shaded(attack[4+(won?1:0)], 0xFFFFFF, 0, SH_p2+25, true);
                    paint_shaded(attack[6], 0xFFFFFF, 0, SH_p2+40, true);
                ...
                case PRE_RUNNING_GAME_SCENE_WITH_CLIENTS:
                    ...
                    paint_gui(false);
                    fill_shaded_rect(0x355046, SW_p2-70, SH_p2-60, 140, 120);
                    paint_shaded(startTxt[3]+server_actround, 0xFFFFFF, 0, SH_p2-55, true);
                    y=SH_p2-35;
                ...
                case PRE_RUNNING_GAME_SCENE_WITH_NUMBERS:
                    ...
                    paint_shaded(startTxt[3]+server_actround, 0xFFFFFF, 0, SH_p2-55, true);
                    y=SH_p2-35;
                    paint_numbers(SW_p2-60, y, server_hg_sajatsorszam, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                    y+=25;
/*                    for (i=0; i<jatekosok_szama; ++i) {
                    ...
                    for (int ii=0, tempi=pre_running_cnt; ii<Math.min(2, jatekosok_szama-1); ++tempi) {
                        ...
                        paint_numbers(SW_p2-60, y, tempi, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                        y+=25;
                        ++ii;
                    }
                ...
                case PRE_RUNNING_GAME_SCENE_WITH_NAMES:
                    ...
                    fill_shaded_rect(0x355046, SW_p2-70, SH_p2-60, 140, 140);
                    paint_shaded(startTxt[0], 0xFFFFFF, 0, SH_p2-55, true);
                    paint_shaded(startTxt[1], 0xFFFFFF, 0, SH_p2-40, true);
                    paint_shaded(startTxt[2]+server_lastround, 0xFFFFFF, 0, SH_p2-25, true);
                    y=SH_p2-5;
                    paint_name(SW_p2-65, y, server_hg_sajatsorszam, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                    y+=25;
/*                    for (i=0; i<jatekosok_szama; ++i) {
                        if (i==server_hg_sajatsorszam) {
                            continue;
                    ...
                    for (int ii=0, tempi=pre_running_cnt; ii<Math.min(2, jatekosok_szama-1); ++tempi) {
                        ...
                        paint_name(SW_p2-65, y, tempi, SQUAD_ANIM_X, SQUAD_ANIM_Y);
                        ++ii;
                        y+=25;
                    }
                    paint_footer("", pause_txt[0], 0xFFFFFF);
...
/*				for (i=0; i<4; ++i) {
                ...
                case WAITING_FOR_RESPONSE_SCENE:
                    ...
                    g.setClip(SW_p2-4, SH-20-2-9, 9, 9);
                    g.drawImage(img[1], SW_p2-4, SH-20-2-9, Graphics.TOP|Graphics.LEFT);
                    
                    g.setFont(fontpsmall);
                    g.setClip(0,0,SW,SH);
                    if (scene==WAITING_FOR_RESPONSE_SCENE) {
                        paint_connection_panel(other_txt[6], SW-30);
                    } else {
                        ...
                        g.setColor(0);
                        g.drawLine(0, SH-20, SW, SH-20);
                        paint_shaded(MISSING_RESPONSES_STRING_1+server_missingresponses+MISSING_RESPONSES_STRING_2, 0xFFFFFF, 5, SH-17, true);
                    }
                    break;
                ...
                case PAUSE_SCENE:
                    ...
                    paint_shaded(other_txt[4], 0xFFFFFF, 0, SH_p2-55, true);
                    g.setFont(fontpsmall);
                    g.setColor(0);
                    
                    paint_menu_txt_center(pause_txt, 0, SH_p2-15, 17, true);
                    break;
            }
        }
